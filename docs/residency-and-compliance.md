...
(edge geo-routing — Slice C, roadmap)
      ...
      DOKS + Istio     DOKS + Istio     DOKS + Istio
            |               |                |
...
| Brazil | — (no DO region) | LGPD | out of scope (§8) |
...
- **No cross-region PII.** Inter-service calls occur only within a region; no service in one region reads another region's database.
...
|---|---|---|---|---|
| Encryption in transit | Istio **STRICT mTLS** between services; **TLS-required** (`sslmode=require`) to managed DB; edge HTTPS (roadmap) | Art. 32 | Principle 7 (Safeguards) | §1798.150 |
...
| Data residency | Cluster + managed DB per jurisdiction; edge geo-routing (roadmap) | Ch. V (transfers) | P1 (Accountability) | — |
| Secrets management | **No plaintext secrets in Git**; DB credentials injected from Terraform state; Vault + External Secrets Operator (roadmap) | Art. 32 | P7 | §1798.150 |
| Supply-chain integrity | CI build → **keyless Cosign signing** (OIDC/Fulcio/Rekor) → **SBOM (SPDX)** attestation → push to ghcr; Kyverno admission verification (roadmap on DOKS) | Art. 32 | P7 | — |
| Change management & audit | Terraform IaC with **stateful/stateless lifecycle split**; **GitOps (Argo CD)**; full Git history of every change | Art. 5(2) Accountability | P1 | — |
...
- **Terraform** — `modules/region-data` (VPC + managed Postgres, long-lived) and `modules/region-cluster` (DOKS + node pool, ephemeral) are called from `environments/{us,eu,ca}/{data,cluster}`. The stateful data layer has its **own state** so clusters can be destroyed/rebuilt without risking data.
...
- **CI** — matrix build across all services → keyless sign → SBOM → attest → publish, so every running image is traceable to a signed commit.
...
- **Brazil / LGPD** — DigitalOcean has **no South America region**, so in-country residency is not achievable on DO. If required: a multi-cloud São Paulo cluster, or LGPD-compliant international-transfer safeguards. Out of scope for now.
...
- *"DOKS is managed control plane + self-managed node pools (GKE-Standard / EKS tier). Karpenter is AWS/Azure-only, so node scaling here is the cluster-autoscaler via pool min/max."*
